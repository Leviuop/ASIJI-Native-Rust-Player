...
```bash
sudo apt install ./asiji_0.9.1_amd64.deb
asiji
```
...
В меню выбора трека нажмите **S**. **↑↓** выбирают параметр, **←→** и **Enter**
меняют значение. **W** сохраняет изменения, **Q** или **Esc** отменяют.
Визуал, палитра, громкость, декодер и режим обоев доступны в этом списке.
Для путей, кэша и биндов нажмите **E**: здесь сохраняется ввод вида
`bind pause=k,space` (пустое значение выключает бинд). Значения и конфликты клавиш
...
Доступен экспериментальный режим [живых обоев](WALLPAPER.md):
в настройках включите «Живые обои», сохраните и выберите трек.
В Linux backend выбирается автоматически или через «Обои: система»;
для своего окружения установите компоненты, перечисленные в руководстве.
