...
- Исправлена отправка Enter в Linux PTY-тесте настроек.
- Первая готовая поставка возможностей 0.9.0 после полной проверки обеих платформ.

## 0.9.0

- Настройки со стрелками: визуализатор, палитра, режим, чувствительность,
  громкость и декодер выбираются без команд; расширенный ввод сохранён на E.
- Экспериментальные живые обои Windows Explorer на основном мониторе:
  видео и визуализатор, ASCII/HD, работа со свёрнутым терминалом.
- Явная надпись при mute с подсказкой текущего бинда. Аудиодвижок не изменён.
- Терминальные тесты сохранения/отмены настроек и локальные тесты окна обоев.
- Linux сохраняет терминальный режим; интеграция живых обоев пока отсутствует.
