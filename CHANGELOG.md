...
- Ускорена отрисовка Orbit за счёт кэша неизменной геометрии.
- Wave рассчитывает положение волны один раз на столбец.
- Добавлены побайтное сравнение визуала с предыдущим рендерером и CPU-бенчмарк
  для Windows/Linux. Качество, цвета и настройки не изменены.

## 0.9.2

- Меню настроек со стрелками на Windows и Linux: выбор визуализатора, палитры,
  чувствительности, громкости и декодера без ввода команд.
- W сохраняет настройки, Esc отменяет; E открывает расширенный ввод путей и биндов.
- Проверки реальных нажатий клавиш, сохранения и отмены в терминале.
- Упрощена поставка: только терминальный плеер и его зависимости.
